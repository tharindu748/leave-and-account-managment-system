      ...
      include: {
        ...
        user: {
          select: {
            ...
            name: true,
            email: true,
            jobPosition: true, // Using jobPosition instead of department
          },
        },
      },
    });
...
// In leave.service.ts - make sure this method exists
async getCalendarData(year: number, month: number) {
  ...
  const leaveRequests = await this.databaseService.leave_request.findMany({
    where: {
      ...
      status: {
        in: [LeaveStatus.PENDING, LeaveStatus.APPROVED, LeaveStatus.REJECTED],
      },
    },
    include: {
      dates: {
      ...
      user: {
        select: {
          ...
          email: true,
          jobPosition: true,
          imagePath: true, // Make sure this is included
        },
      },
    },
  });
  ...
  const calendarData = leaveRequests.flatMap(request => {
    const status = request.status || LeaveStatus.PENDING;
    ...
    return request.dates.map(date => ({
      ...
      userImage: request.user.imagePath, // Include image path
      leaveType: request.leaveType,
  ...
  async cancelLeaveRequest(dto: CancelLeaveRequestDto) {
    ...
    return this.databaseService.leave_request.update({
      where: { id: dto.requestId },
      data: { status: LeaveStatus.REJECTED, approvedBy: dto.approvedBy },
      include: { dates: true, user: true },
    });
  }
  ...
  // Add test endpoint implementation
  async testEndpoint() {
    return { 
