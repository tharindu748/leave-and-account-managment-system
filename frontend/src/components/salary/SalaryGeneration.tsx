// // src/components/salary/SalaryGeneration.tsx
...
// import { Loader2, Download, Calculator, Search } from "lucide-react";
...
//     const res = await axios.get(`http://localhost:3000/salary/period?${params}`);
//     console.log("Generated salaries response:", res.data);

//     // ✅ FIX: use the correct array depending on backend structure
...
//     setGeneratedSalaries([]); // prevent crash if error
//   }
// };
...
//   const calculateSalary = async (employeeId: number) => {
//     try {
//       const res = await axios.get(
//         `http://localhost:3000/salary/calculate/${employeeId}/${selectedMonth}/${selectedYear}`
//       );
//       return res.data;
//     } catch (error) {
...
//     try {
//       const res = await axios.get(
//         `http://localhost:3000/salary/period/${selectedMonth}/${selectedYear}`,
//         { responseType: 'blob' }
//       );
...
//       link.href = url;
//       link.setAttribute('download', `salary-sheet-${selectedMonth}-${selectedYear}.csv`);
...
//     salary.user?.epfNo.toLowerCase().includes(searchTerm.toLowerCase())
//   );
...
//               <Button
//                 onClick={() => generateSalary(selectedEmployee === "all" ? undefined : Number(selectedEmployee))}
//                 disabled={generating}
//                 className="w-full"
//               >
//                 {generating ? (
...
//             <Button onClick={downloadSalarySheet} variant="outline">
...
//       <Card>
//         <CardHeader>
//           <CardTitle>Generated Salaries - {months.find(m => m.value === selectedMonth)?.label} {selectedYear}</CardTitle>
//         </CardHeader>
//         <CardContent>
//           {loading ? (
...
//               No salaries generated for {months.find(m => m.value === selectedMonth)?.label} {selectedYear}
//             </div>
//           ) : (
...
//                       <TableCell className="font-medium">{salary.user?.name}</TableCell>
//                       <TableCell className="font-mono text-sm">{salary.user?.epfNo}</TableCell>
//                       <TableCell>{salary.user?.jobPosition}</TableCell>
//                       <TableCell>LKR {salary.basicSalary.toFixed(2)}</TableCell>
//                       <TableCell>LKR {salary.overtimePay.toFixed(2)}</TableCell>
//                       <TableCell>LKR {(salary as any).allowance?.toFixed(2) || "0.00"}</TableCell>
//                       <TableCell>LKR {salary.leaveDeductions.toFixed(2)}</TableCell>
//                       <TableCell>
//                         <Badge variant={salary.totalLeave > 0 ? "destructive" : "default"}>
//                           {salary.totalLeave} days
//                         </Badge>
//                       </TableCell>
...
//                     <TableCell>LKR {emp.otRate.toFixed(2)}/hr</TableCell>
//                     <TableCell>LKR {emp.allowance.toFixed(2)}</TableCell>
//                     <TableCell>LKR {emp.deduction.toFixed(2)}</TableCell>
//                     <TableCell className="font-semibold">LKR {emp.netSalary.toFixed(2)}</TableCell>
//                   </TableRow>
//                 ))}
//               </TableBody>
//             </Table>
